...
import { ElementInfo, RelationshipAttribute, SlideInfo, TemplateInfo, XmlDocument, XmlElement } from './xml-types';
...
export type ModificationCallback = ChartModificationCallback | ShapeModificationCallback;
export type ShapeModificationCallback = (element: XmlElement, relation?: XmlElement) => void;
export type ChartModificationCallback = (element: XmlElement, chart?: XmlDocument, workbook?: Workbook) => void;
...
export type PresentationInfo = {
    ...
    elementByName: (tplName: string, slideNumber: number, elementName: string) => ElementInfo;
};
export type Target = {
    file: string;
    type: string;
...
export type TrackedRelation = {
    ...
    type?: string;
    attribute?: string;
    role?: 'image' | 'slideMaster' | 'slide' | 'chart' | 'externalData' | 'slideLayout' | 'hyperlink';
    targets?: Target[];
};
...
export type GenerateOnSlideCallback = (pptxGenJSSlide: IPptxGenJSSlide, pptxGenJS: PptxGenJS) => Promise<void> | void;
...
export type ImportedElement = {
    ...
    sourceRid?: string;
    sourceMode?: 'image:svg' | 'image:media' | 'image:audioFile' | 'image:videoFile';
};
